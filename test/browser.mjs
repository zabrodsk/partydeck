...
try{
 ...
 const byId=new Map();for(const p of[host,friend,third])byId.set((await read(p)).room.me.id,p);
 ...
 for(const name of ['Sam','Jo','Nora','Max','Ali']){const p=await page();await p.goto(base+'/join/'+code);await p.locator('[name=name]').fill(name);await p.getByRole('button',{name:'Join table'}).click();await ready(p);}
 await host.getByLabel('Close dialog').click();await host.waitForFunction(()=>JSON.parse(window.render_game_to_text()).room.players.length===8);await clickCommand(host,'[data-command=start]');
 ...
 await clickCommand(mobile,'[data-command=start]');assert.ok(['playing','complete'].includes((await read(mobile)).room.phase));
